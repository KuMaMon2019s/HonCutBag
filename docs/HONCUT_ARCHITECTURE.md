...
Seedance 2.0 普通主镜头使用固定的全模态参考合同：canonical visual contract 先作为结构化 Prompt 权威注入；实际媒体顺序为当前 Pxx 可见角色各一张四视图身份板 → P01 的 Phase 4 成片首帧或 P02+ 的真实前序视频 → 当前动作局部图 → 身份中立剧情导航图 → 必要尾帧锚点。所有普通输入图片写为 `role=reference_image`，连续前序媒体写为 `role=reference_video`；必要资产超过九张图片时提交前 fail closed，不得静默裁减。Prompt 必须在最终 `content[]` 顺序确定后再生成一基“图片 N / 视频 N”索引，不假设首帧固定为图片1。导演总览与导演 Sxx 单格只用于生成干净首帧，不直接进入 Seedance；Pxx PREVIS 也不进入 Seedance。Phase 2 的审核九宫格保留 `Sxx_G01～G09`、红色主体/物体动作箭头、蓝色运镜箭头和空间/视线/动作指示标识，但 Phase 6 只接收从 canonical action-unit 与 body-action mechanics 本地编译、由固定几何 renderer 绘制的身份中立 `storyboard_narrative_guide` v4。当前 `honcut.storyboard-guide-pose-contract.v3` 与 pose policy v3 必须按 technique、footwork、torso、weight shift、end pose、canonical action、contact 的受控优先级选择动作族，并识别中文和英文局部否定，使“无格挡”“未击中”“without blocking”等词只能成为审计中的 rejected match，不能覆盖正向动作。body-action beat 仅可影响 source micro-action index 匹配的 generation unit；同一动作扩展到多个 Gxx 时必须保存单调 pose progress，并以根位移、重心下沉、躯干倾斜、步幅、支撑脚和双手握持共同驱动关节几何，确定性校验必须拒绝只有箭头变化而身体位移不足的产物；相邻 action unit 必须从前一动作的最终关节和累计根位置继续，并绑定前一 unit 的 transition origin，禁止动作边界回到中立站姿。P01 首个纯 `ready` 动作只有在后续存在动态动作且 Phase 4 cinematic first frame 已建立该姿态时，才压缩为唯一 `timing_role=initial_anchor`、`story_time_weight=0` 的 Gxx；它保留完整 canonical 血缘但不占成片动作时长，Phase 6 必须要求首帧后立即执行下一 Gxx。单独戒备、P02+、持道具和空间状态不得自动压缩。最终索引绑定必须附加媒体职责：导航图只拥有 Gxx 顺序、逐格关节姿态、几何轨迹、箭头和空间关系，不拥有人物脸、发型、妆造、服装、体型或道具外观；角色身份只服从四视图板，开场/连续状态只服从成片首帧或真实前序媒体，当前姿态、握持和道具几何只服从当前动作图及 canonical 文字。动作姿态图中的多个人形必须解释为同一角色的参考姿态，只允许执行当前 Pxx 声明的有序 Axx，禁止生成克隆、分栏、网格、文字、序号、箭头、边框或提前演绎后续动作。`prop_detail_board`、`identity_detail`、历史剧情状态图和 guide v1/v2/v3 均不得进入视频媒体。authority manifest、最终职责/索引、全部媒体 hash、Prompt hash、Gxx/Axx、zero-time anchor cells、pose contract/policy hash、逐格 pose fingerprint 和 canonical hash 必须进入任务 fingerprint。只有需要严格端点像素约束的 FLF2V 与跨镜 bridge 才使用 `first_frame/last_frame`，且该请求不得混入剧情九宫格、角色参考、动作姿态图或其他 `reference_image/reference_video`。
...
Phase 2 的 Pxx、模型绘制 Sxx 审核九宫格与跨一级镜头 bridge 都必须在相同语义输入的普通重入和 Phase 3 character-lock 刷新中复用。每个 Sxx 九宫格固定 3×3，九个格子依阅读顺序绑定唯一 `Sxx_G01～G09`；Phase 2 按已声明 Pxx 顺序确定性、完整且无重叠地分配九格。Phase 2 pose compiler 将每个 Gxx 绑定当前 Pxx 的有序 generation/source action-unit、performer/target、阶段、时序角色与匹配的 body-action mechanics，再由 `honcut.identity-neutral-story-guide-renderer.v2` 按归一化关节几何绘制 `honcut.storyboard-narrative-guide.v4`：不同的动作或非静态阶段不得塌缩为同一 pose fingerprint；多人交互使用确定性角色槽位，非人物运动使用抽象物体符号，禁止凭空生成人形。红箭头只来自已解析动作向量，蓝箭头只来自 camera-motion contract；renderer 不复制审核板源像素，不绘制脸、发型、服装纹理、道具外观或剧情文字。shot-storyboard manifest v6 必须记录 pose schema/policy hash、逐格 pose contract/fingerprint、零时长锚点列表、语义 payload hash、Gxx→Pxx 分配、源审核板 hash、派生图 hash 与 `source_pixel_usage=none`。guide v1/v2/v3 只有在完整九格合同、源 hash、Pxx 分配、收据与 canonical action lineage 全部可验证时，才允许零调用并排归档旧证据后重绘为 v4；否则旧资产只能 audit-only 并要求从 Phase 2 重建，未知未来 schema 一律 fail closed。Sxx cache 至少绑定当前 Prompt SHA-256、模型、尺寸、宽高比、图片请求合同版本、结构化 Phase 5 纠偏 attempt/issue、可解码文件、内容 SHA-256 与完整 grid receipt；任一项变化或损坏时只重生成该 Sxx，不得连带重提已验证的 Pxx。
