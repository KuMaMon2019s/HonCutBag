...
Phase 3 动作板修复复用专用入口 `pipeline/scripts/phase6_storyboard_guide_live_acceptance.py`，但收据合同升级为 `honcut.phase3-performance-board-live-acceptance.v1`：它只从至少含两个 Pxx、P01 可见角色为一至两人、且 P01 存在绑定道具的当前动作图之 Sxx 投影一个请求。零提交预检必须验证 v3 合成人身份、整板/局部图/收据哈希、最终媒体顺序、Gxx/Axx/source action-unit/道具、Prompt、预算和 generation fingerprint；隔离 `GenerationTaskStore`、逻辑 submit guard 与原始 Seedance POST guard 共同限制为最多一次真实提交。regression evidence 可在未提交时独立落盘为 pass，整体仍保持 `pending_live_acceptance`；调用链成功后仍保持业务 verdict 待人工审查。只有同一 Git 提交的 regression evidence 与“从指定首帧开始、珍珠生体瓷妆明确且美观、人物一致不克隆、道具正确、只演绎当前 Gxx/Axx、成片无文字/箭头/边框/网格”的业务 verdict 均为 pass，Phase 1～Phase 9 收据才可写为 `accepted`。
