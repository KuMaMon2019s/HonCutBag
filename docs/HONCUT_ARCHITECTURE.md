...
| 超时、重试、冷却与容量 | `pipeline/src/runtime/provider_policy.py`、`pipeline/src/runtime/llm_policy.py`、`pipeline/src/runtime/provider_attempt_policy.py` | Provider、Graph 和 Phase 不得叠加重试；健康 LLM 长流由 idle 与 wall 两个时钟区分；专用真实验收进程可用该 Runtime 作用域统一收紧所有工作线程的尝试上限，并通过 `utils/provider_request_guard.py` 的窄传输协议落账，client 不得反向依赖 Runtime |
| 长任务账本 | `pipeline/src/runtime/generation_tasks.py` | SQLite 幂等迁移、提交去重、恢复与终态证据 |
| TOS 输入上传账本 | `pipeline/src/runtime/tos_uploads.py` | Runtime 按最终 payload 配置超时和 PUT 硬上限；`TOS_UPLOAD_LEDGER.json` 持久化一次提交与只读对账，client 只执行签名 HEAD/PUT |
...
| 2 | Pxx PREVIS、每个 Sxx 独有的带标识 3×3 审核九宫格、Gxx→Pxx 分配，以及本地绘制的身份中立导航图 v3 | 生产图片、格子分配、canonical action lineage、pose contract、canonical hash 或导航血缘证据缺失时 fail closed |
...
Phase 2 的人物姿态只消费 Phase 1 已确定的实例身份：当前 Pxx 的 canonical instance ID、角色资产显示名/已登记 aliases/source mentions，以及 `participant_refs` 中明确绑定该实例的 mention，必须投影为同一个 canonical actor role。别名碰撞直接 fail closed，Phase 2 不得凭文本相似度推断、合并或增加人物。包含 canonical performer 的动作样本必须同时具有非空 `actor_roles` 与人物骨架几何；回归测试必须在排除 Gxx 标签和红/蓝箭头区域后验证实际人体栅格随动作阶段变化，不能只比较语义 hash。
...
Event Extractor 必须为每条来源 micro action 输出角色、目标、动作类型、节奏、状态读写以及 `after/overlap/reaction_overlap/effect_of/sustained_during` 关系；代码在索引、覆盖、引用方向和枚举等确定性结构校验通过后写 `honcut.action-timeline.v1`。`phase1_semantic_qa` 控制时间关系与动作类型组合、同角色并行动作、交互式攻防及 ensemble 贡献等概率性语义判断：新运行默认关闭，关闭时模型给出的任一结构合法组合都不得阻断；Event owner 只为排程安全执行确定性归一化，并以 `honcut.action-temporal-reconciliation.v1` 保存原值、规范值、受控原因、证据哈希和 Policy 哈希。主动动作误写为零时长关系时保守提升为新的有序时间片，避免丢失动作；被动环境/持续状态误写为 actor overlap 时附着为零故事时间，非 choreography 的被动执行者移入 targets；一个 overlap/zero-time 动作合法引用多个已物化时间片时，默认模式确定性附着到最新时间片，保留完整原始引用并记录实际调度锚点，严格模式继续阻断。所有归一化均写入 `diagnostic_only`，不增加 Provider 请求；显式开启语义 QA 后才按严格语义合同 fail closed。该开关必须进入 CLI/Lifecycle、run manifest、Graph State、Phase 1 checkpoint 与 segment cache fingerprint，Graph 和顺序执行器调用同一 Event Extractor owner。攻击与同步闪避/格挡应位于同一 event 的 `reaction_overlap` 时间片；位移后抓取、控制后投掷、回合制反击与慢节奏检查/揭示应保持串行。被动粒子、火花、碎片、玻璃震动和持续环境可附着时间片而不单独占故事时间，但来源索引和哈希必须永久保留。关闭语义 QA 不得放宽 JSON schema、micro-action 完整覆盖、引用越界/前向引用、非法枚举、哈希、血缘或 Provider 预算校验。
...
Character Roster v2 对编号人物的等价归并增加受控规则：中文序号、`第1名` 式阿拉伯数字前缀和中文人物词干后的阿拉伯数字后缀，只能在同一计数群体已唯一成立、序号完整、称呼不共现且数字后缀没有早于群体声明时按 `(词干, 序号)` 等价。等价关系写入 `identity_reconciliations`；普通名称末尾数字不得脱离计数群体自行触发归并。该行为属于角色缓存语义，当前 `CHARACTER_CONTEXT_SCHEMA_VERSION=16`，v15 角色上下文不得满足新运行复用。

Phase 1 的生产顺序固定为 `text_parser → Event Extractor → Director Planner → character discovery → Adaptation Engine`。Director 不得重新分场；`director_plan.json` 使用 `honcut.director-plan.v1`，必须按事件账本顺序对每个且仅一个 `sequence_id` 写入 `scene_goal`、`emotion_arc`、`visual_focus`、`spatial_intent` 与 `transition_intent`。Director 请求必须使用 Chat Completions 原生 `response_format=json_schema` 并通过 strict `DirectorPlanUnderstanding` DTO；JSON/schema 或 sequence 覆盖失败只能触发一次有界结构纠正，仍失败则 fail closed。来源事件账是 sequence ID、数量和顺序的唯一权威，模型返回的五项导演意图只是 Observation；模型为同一 ID 连续返回多条完整意图时，Director owner 必须保留第一条、把后续项作为非权威替代提案按位置和哈希写入 `honcut.director-plan-reconciliation.v1`，不得拼接 prose 或增加 Provider 调用。折叠后仍须与来源 sequence 顺序完全一致；非连续重复、缺失、未知、乱序或任一意图为空继续 fail closed。收据只保存来源、Observation、canonical intent、Prompt、schema 和 policy 哈希，不保存模型正文。Director 只说明“为什么这样拍”，不得决定镜头数量、景别、机位角度、运镜、焦段、光影或时长；这些具体字段仍由 Adaptation Engine 唯一拥有。Director 意图属于 adaptation 的语义输入，必须进入分层 checkpoint fingerprint；意图变化不得复用旧 beat/shots checkpoint。sequence 级原始 Director 意图可能包含随后被时长压缩删除的事实，只能保存在 `director_plan.json` 和 source fingerprint 中；生产 beat、canonical shot、监督投影与 Provider Prompt 必须改用 `honcut.production-director-intent.v1`，其 `scene_goal/emotion_arc/visual_focus/spatial_intent/transition_intent` 只能由该镜 `source_event_ids` 指向的 canonical 事件确定性投影。生产 beat 的 `who/where/what/reason/texture_keywords` 同样必须在确定最终 `source_events` 后由这些事件重新物化，禁止保留骨架模型基于完整 sequence 产生的自由文本。投影 schema、sequence 或 event lineage 不匹配时 fail closed，禁止把原始 sequence 文案整段复制到生产镜头而重新引入 `whole_event_omitted` 内容。
...
Phase 4 每个一级镜头 Sxx 只生成一张 cinematic first-frame，并绑定 P01；P02+ 从上一段真实视频末态延长，不得另生成首帧造成姿态或剧情复位。该首帧收据必须同时记录不含 L4 纠偏覆盖层的基础输入指纹和实际 Provider 输入指纹。某次 L4 拒绝已由收据中的报告哈希明确 supersede、而后续 Phase 5 报告已通过或不再拒绝该帧时，只要当前基础指纹与收据基础指纹一致，必须复用该纠偏成功帧并保留实际生成 Prompt，不得因纠偏文字从当前 Prompt 消失而再次付费生成；模型、尺寸、参考图、角色/场景合同或新的拒绝上下文发生变化时仍必须失效缓存。已知 v1 运行中 P02+ 的历史 cinematic 文件保留作审计，但迁移后不得再写入 canonical beat 或 continuity plan。
...
Phase 1 的事件、角色、时长缩放动作选择与 Storyboard Prompt 请求必须使用 Chat Completions 原生 `response_format=json_schema`，并分别通过 strict `EventUnderstandingBatch` / `CharacterRosterObservationBatch` / `DurationScaledActionSelectionBatch` / `StoryboardPromptUnderstanding` DTO。所有结构化模型文本统一进入 `schemas/understanding.py::parse_structured_output()`：先保留标准 `json.loads` 快路径，只有语法失败时才允许 `json-repair` 修复单一文档中的标点或闭合分隔符，随后仍必须通过 strict Pydantic。统一 parser 必须拒绝前导/尾随 prose、第二个顶层对象、未闭合字符串、停在 `{` / `[` / `:` / `,` 的不完整值以及无法满足 DTO 的修复结果；Phase 不得自行扫描 JSON 片段、截断、拼接或绕过 schema。模型产生的 `who` 是来源动作主体：需要人物身份资产的称呼在角色发现完成后必须确定性绑定到唯一 canonical `character_id`，生成 `honcut.semantic-understanding.v3` 的 `SEMANTIC_LEDGER.json`；可确定为车辆、环境或道具的非人物动作主体继续原样保存在事件 `who`，并写入 `non_character_participants` 审计字段，但不得进入角色名册、身份板或 Phase 6 人物媒体。未知或疑似人物的未绑定称呼仍必须 fail closed，只有来源以代号、姓名或昵称明确声明时，非人物字面标签才可提升为人物身份。v3 采用混合语言合同：原始 `source_excerpt`、来源称呼和对白逐字保留且不得由翻译覆盖；身份连接只使用语言无关的 `character_id` / `ref_id`；`entity_type`、`gender`、`role` 和来源语言只使用英文受控枚举。`男性/女性/male/female` 等性别属性称呼不是独立角色身份；只有同一来源账中存在唯一、等价且不共现的稳定人物称呼时，Character Discoverer 才可把属性称呼保留为该角色的 source alias，多候选或共现必须继续 fail closed。Provider 的语言化 Prompt 是可重建投影，翻译不得改变身份、事件顺序、动作血缘、时长或逐字对白。结构化 `body_action_choreography.performer` 是动作所有权证据：singular performer 缺失于 `who` 时，Event Extractor 必须在审计字段中记录并补入参与者集合；未知、代词或群体 performer 不得借此创建角色。舞蹈、格斗与武术事件的每个身体动作必须映射到结构化 choreography beat；一旦模型或上游声明任何人体 choreography，即使周围文案没有“格斗/舞蹈”标签也必须强制校验，`performer/technique/side/limbs/footwork/torso/weight_shift/direction/contact/end_pose` 必须全部是非占位的可执行值。身体动作需求以当前 `micro_actions`/`generation_actions` 账优先，`what/visual` 只在没有当前动作账时作上下文回退；模型误放进 body DTO 的武器撞墙、冲击波、灯光、玻璃、雨滴等物体或环境结果必须从 body score 确定性剔除，但继续原样保留在 canonical 动作账及其血缘中，不得因背景仍提及格斗而伪造人体字段或缺拍。`contact` 表示这一拍的接触状态而非强制命中：不含撞击、抓握、格挡、踢打等接触语义的位移或闪避若返回空值/`无`/`不适用`，必须规范为“无目标接触；身体保持既有支撑接触”；包含接触语义的动作仍须给出具体接触点，禁止借该规则伪造无接触。`side` 是生产编排而非来源剧情事实：模型提供具体侧别时原样保留；为空或含 `未明确/未指定/unknown` 时，Event owner 必须用稳定的执行者与当前动作指纹确定性选择左侧或右侧并明确标注“确定性导演编排”，同一输入重跑不得漂移，且不得把该选择反写为来源事实。Event Extractor 对动作索引、动作覆盖和来源事实继续 fail closed；默认关闭语义 QA 时，模型完全遗漏或部分遗漏 body choreography，以及任意数量的 `performer` 以外生产编排字段，都不得触发第二次付费提取。Event owner 只有在每个身体动作已绑定来源索引，且执行者可由 action temporal relation 或唯一来源称呼确定时，才按缺失索引合并待补全 beat，以 `honcut.body-action-placeholder-reconciliation.v1` 保存缺失索引、来源证据哈希和 Policy 哈希，并写入 `body_action_director_repairs_pending`；已存在的模型 beat 不得改写。执行者无法唯一确定、索引损坏、重复或动作未覆盖仍阻断。显式严格模式仍拒绝此类模型遗漏。Director Planner 必须在任何导演模型请求前确定性补全这些字段、重新验证完整 body contract，并把精确动作索引与补全字段写入 `body_action_director_repairs`；Screenwriter 随后重写 `phase1_events.json` 保存完成态。执行者无法从来源唯一确定、动作索引或覆盖损坏、以及确定性补全后仍无效继续 fail closed，禁止把坏账推迟到 Phase 5。相邻连续事件若仅把同一人物退化为等价的人类描述词，可在来源没有“另一名/新角色”等引入证据时继承前一稳定身份；当前事件退化为短泛称而下一连续事件恢复具体身份时，只有该具体身份同时逐字存在于当前事件自身的 `source_excerpt/what/start_state`、且跨事件候选唯一，才允许前向回填并保存模型原始值和审计映射；无当前来源证据或多人候选必须 fail closed。连续动作中遗漏的受力/攻击对象只能在“前一事件 canonical 参与者、当前 `continuity_subject`、当前 `source_excerpt/what`”三处逐字证据一致时补入 `who` 并记录审计字段；拉远镜头等仅在 continuity subject 出现但当前画面未点名的角色不得提升为可见参与者。Event Flow 只裁决来源语义并行、因果与角色冲突，不得用具体视频模型的单次运动容量拒绝真实的同时发生关系；超载语义时间片由后续 timeline binding 拆成共享 `semantic_temporal_slice_id` 的相邻 Provider 执行子片。Segment cache 在复验前必须剥离 `body_action_contract`、ID、sequence 等规范化派生字段，只把原生 DTO 字段重新送入 strict parser，避免有效非空缓存因内部字段被误判而重复付费请求。Event Extractor 使用 Runtime 的 32k 长结构化输出策略，并要求同一事实只在职责字段中写一次；不得通过拆分 target、删减来源动作或增加请求来规避结构化响应长度。未闭合字符串和值截断仍必须 fail closed，在零重试验收作用域内不得重提。上述规则属于事件缓存语义，当前 `EVENT_FLOW_SCHEMA_VERSION=33.0`；v31 及更早 segment cache 不参与生产复用，新 run 必须重新提取事件。抽象角色词默认不是人物资产；只有源文本以代号、化名、姓名或昵称显式声明时，角色 owner 才可基于保存的 `source_excerpt` 将其提升为稳定身份，禁止为具体剧本标签建立白名单。Character Roster owner 必须在角色模型调用前从来源事件原子写入 `honcut.character-roster.v2` 的 `CHARACTER_ROSTER.json`，以来源事件哈希、稳定 entity/instance ID、数量、source mention、event/action-unit refs 、限定人物称呼 reconciliation 与视觉证据血缘固定人数和归属；限定称呼只在同一连续 sequence、候选唯一、来源文本交叉印证、从未共现且没有新角色或序号引入证据时归入同一实例，两个限定描述或多候选不得仅凭共享性别合并；群体归并只在明确数量、完整编号、同一连续 sequence、数量一致且没有竞争群体时成立；计数群体建立后，后续无序号泛称只有在其词干与完整编号成员一致、位于声明之后、候选群体唯一、来源逐字存在、未与编号成员共现且没有“另一名/新角色”证据时，才可确定性绑定到该群体的全部实例。声明前已存在的同名个体保持独立；无群体证据的编号人物保持独立，数量冲突、重复序号、多群体歧义或泛称共现歧义 fail closed。Character Discoverer 只允许一次 `honcut.character-roster-observation.v1` 结构化调用，必须回传当前 roster hash 且只能补充既有 entity 的视觉事实；无效 JSON、错误或未来 schema、hash/来源血缘损坏始终阻断且不得重提。默认关闭的 `phase1_semantic_qa` 将可确定修复的 alias 遗漏、角色定位偏差以及错误拆分/合并写为诊断并由 Roster owner 重建；显式开启时同类偏差阻断，人数、序号、来源覆盖、hash 与 lineage 在两种模式下都严格。该角色上下文合同当前为 `CHARACTER_CONTEXT_SCHEMA_VERSION=16`；`honcut.canonical-visual-contract.v2` 与 `honcut.semantic-understanding.v3` 分别保存共享 entity 和独立 instance，下游 Phase 2～6 只以 instance ID 绑定镜头、身份板、连续性和视觉证据，名称只作展示。

`CHARACTERS.json` 必须分别输出共享 `entities` 和按 instance 展开的 `characters`；Phase 3 的角色资产规格、项目注册表指纹和缓存身份必须同时包含 entity ID、instance ID 与 canonical hash，共享实体只共享服装、装甲和道具事实，不得让不同 instance 复用同一脸部资产。Phase 6 必须为当前 Pxx 的每个可见 instance 加入对应身份板，并在最终媒体编号后验证九图上限，超限时在付费前 fail closed。一对一 `honcut.canonical-visual-contract.v1` 只有在完整 source mention 与 event lineage 可验证时才能零请求迁移为 entity=instance，并写独立 `honcut.canonical-visual-contract-migration.v1` sidecar，不得覆盖旧文件；群体、缺失来源血缘和未知未来版本只能标为 audit-only 并要求从 Phase 1 重建。

Phase 1 的 `utils/body_action_contracts.py` 是 canonical 人体运动学的唯一 owner。现行 `honcut.body-action-choreography.v2` 在既有 Provider `BodyActionUnderstanding` 字段完全不变且不增加模型请求的前提下，为每个 `micro_action_index` 编译 `honcut.canonical-action-kinematics.v1`；最终 GAU/Pxx 确定后，同一 owner 再生成 `honcut.generation-action-kinematics.v1` 投影。坐标固定为演员局部系：+X 为演员右侧、+Y 向上、+Z 向前，Pxx 起始姿态是相对 yaw 0；摄影机相对视图不得改写演员朝向。每个 performer 必须拥有独立且有序的 root、腰/躯干、头、左右臂/手、左右腿/脚完整轨道，动作阶段只细化既有 GAU，不增加故事动作、时长或容量。接触只属于明确的峰值接触阶段；相邻 GAU 继承终态坐标和 yaw，不继承瞬时接触或激活状态。翻转/旋转只有来源动作或已验证 mechanics 明确要求时才可形成受控 transform；运镜、镜像、普通闪避和腰部转动不得提升为翻转。

Phase 2 pose contract v4、Phase 3 performance pose contract v3、Phase 6 action-execution brief v2 与隔离的 motion-blueprint v6 都只消费上述同一投影及 hash，不得重新解析自由动作 prose 或维护平行动作模板。Phase 2 必须把通道数值实际投影到人物骨架像素，并要求每个含 body-action 的 GAU 都有同 beat、同 unit、同 source index 的独立 projection；Pxx aggregate、Gxx action binding 与实际采样的 projection hash 必须逐项一致，组内部分缺失不得由其他 unit 的投影掩盖。Phase 3 只输出当前 performer/GAU 的局部姿态约束；Phase 6 仅投影紧凑的有序 phase、active channels、朝向、transform 与 hash，并保持既有媒体职责和九图预算。motion blueprint 仍是零费用隔离能力门，不得作为普通 Phase 6 `reference_video` 或其他媒体进入生产请求；旧 technique registry 只能作为 immutable audit compatibility。已知 v1 body-action 只有在父 Event/Storyboard Artifact、完整 mechanics、source hash 与最终 GAU/Pxx 血缘全部可验证时，才允许零 Provider 生成并排 `honcut.action-kinematics-sidecar.v1`；`runtime.action_kinematics_migration` 必须先由 `ArtifactManifestStore` 验证父内容、canonical authority 和所有声明的下游血缘，再原子登记 sidecar 与迁移收据，模块命令入口只执行这一零 Provider 路径。原父资产保持不变并转 audit-only，其下游 pose/atlas 资产标记 stale/audit-only。证据不完整、损坏或未来 schema 必须要求 owner 重建，禁止从 prose 猜测或局部刷新。
...
Adaptation Engine 是具体镜头语言的唯一 owner。除 `shot_size`、`camera_movement`、`lens_mm`、`lighting_key` 等既有字段外，canonical shot 必须写入受控 `camera_angle`（`eye_level/low/high/dutch/over_shoulder/aerial/bird/worm`）；Director 意图不得预选该值。下游 Storyboard、二级 Pxx（当前 `honcut.secondary-storyboard.v17`）、Phase 4 元数据与生成 Prompt 只能保留或翻译这个 canonical 值，不得再次推断角度。每个 canonical shot 必须按 `source_events` 顺序持久化 `source_event_casts`，并以 `source_action_unit_refs[{source_event_id, action_unit_id}]` 保存 AU 的强类型事件归属；Storyboard Generator 必须原样复制这些结构化血缘，不得通过字段白名单丢弃。Adaptation 汇总多个来源事件时，镜头角色全集必须直接对 `SEMANTIC_LEDGER.json` 已绑定的 instance ID 做有序去重并验证实例投影；同一群体的展示名与一至多个成员称呼可以在来源索引中重复，但不得重新按名称推断人数、把群体展示名当作第五个实例或覆盖来源 ID。名称只由最终 instance ID 投影为展示字段，未知 ID、冲突 ID 或缺失实例投影继续 fail closed。新 v7 运行的 Pxx planner 必须直接消费 layout 中逐 Sxx 的 `content_beat_counts`、`effective_story_durations_s`、`provider_request_durations_s` 与 `generation_action_unit_capacities`，生成数量必须与声明精确相等；每个 Pxx 与 planning receipt 都必须写入同一 `primary_shot_layout_sha256`。只有缺少新 handoff 的历史 `cut-driven` artifact 可以继续使用确定性的 3-Pxx 推导。每个 Pxx 的 `micro_actions`、`generation_action_units`、`source_event_ids`、`source_action_unit_ids` 与 `character_ids` 必须从同一个 generation-unit bucket 派生；`ledger_indexes` 决定完整来源动作账的连续分区，并把不消耗动作容量的镜头运动或环境状态留在相邻 canonical bucket，禁止分别切分这些数组后再按位置拼接。携带 AU 但无 GAU 的静态、出现或发现事件不增加动作容量，必须按事件顺序归入相邻 canonical bucket，并从 `source_action_unit_refs` 恢复 AU 和角色血缘，禁止从扁平 AU 数组猜测。隐式动作继承对应事件 cast；二人事件保留双方；多角色事件中的 action unit 若显式点名严格子集，只投影该子集，禁止把同一来源事件中尚未执行本格动作的其他角色及其道具提前带入 Pxx。Sxx 级角色全集始终只是上限。来源事件、动作单元、角色 ID、ledger index 缺失/重复/越界、分区交叉或血缘不一致必须在 Phase 1 fail closed。Phase 2 只接受当前 v17 canonical cast；v11 仅可走既有显式可见事实迁移，已知可能漏掉隐式主语、按事件过度扩张角色或丢失 AU 事件映射的 v12/v13/v14 与未知版本即使带有 `character_ids` 也必须 fail closed。生产代码只写 v16 canonical 字段。
...
Phase 5 对标识的判定必须拆成两个互不替代的规则：L3 把当前 Sxx 剧情九宫格作为独立输入，要求可见且正确的 `Sxx_G01～G09`、红色动作方向箭头、蓝色运镜箭头和空间/视线/动作指示标识；L4 只审查每个 Sxx 的唯一 cinematic P01 首帧，要求序号、文字、箭头、轨迹、边框和网格全部不可见。L3 不得把九宫格必需标识误报为成片污染，L4 也不得用九宫格存在来证明干净首帧通过。
...
自动复用只允许同一 `project_id`、同一 canonical `character_id` 和同一版本化静态角色规格 fingerprint 的 `canonical_approved` 资产包。Phase 3 的 A 级只是必要条件；四个 canonical 视角、逐视角 QA receipt、确定性 2×2 参考板 receipt、当前角色卡合同、审批单和每个文件 SHA-256 必须同时有效。命中后先把完整资产包按哈希导入当前 run，再由共同 Phase owner 继续质量门；相同规格出现不同已批准像素时禁止任意挑选并 fail closed。`performance_reference_board.png` 是绑定当前 run 的 Pxx/action-unit 剧情资产，绝不进入跨 run 静态角色注册表；旧 `variant_*.png` 只保留审计，不提升、不扫描、不自动复用，避免把淋湿、破损或泥污等剧情状态误当静态身份。
...
Seedance 2.0 基础镜头提示词的生产 owner 是 Phase 6 `video_generator.build_video_prompt()`；不使用 pose atlas 的兼容路径继续原样透传 `[honcut-video-generation-contract-v2]`，模型 router 不得重排、摘要或追加重复元数据。使用 pose atlas 的请求则由 Phase 6 在最终媒体索引冻结后编译唯一的 `honcut.action-execution-brief.v1`，并以 `honcut.phase6-prompt-projection.v1` 取代基础长文进入 Provider：固定顺序为媒体索引与职责隔离 → 当前 Pxx 动作执行简报 → 紧凑 canonical 身份投影 → 输出约束 → 能整体容纳的场景/情绪/声音可选段。该投影不得任意截断字符串；动作、身份和输出约束任一必需段无法容纳时必须在任务提交前 fail closed。完整基础 Prompt、canonical 合同、动作血缘和媒体合同仍以 hash 进入任务身份与收据，但不得再次作为重复自然语言附在 Provider Prompt 尾部。两种路径都不得使用 `0–3 秒`一类逐子镜硬时码，不得用抽象情绪替代可见表情/呼吸/重心变化，不得在一个镜头同时指定多种主运镜。台词、音效和音乐分别使用 `{}`、`<>`、`（）` 标记；除非剧本显式要求可见文字，必须明确约束无字幕、无 Logo 与无水印。真实输出分辨率只由 `media_profile` 经 Runtime 映射为 Provider 参数，提示词只描述“高清细节”等视觉质量，不得用 `4K` 文案覆盖或暗示与请求参数不同的分辨率。
...
TOS 媒体对象采用内容寻址键时，basename 必须精确等于实际上传 payload 的 SHA-256。Transport 可先用同一凭据签名 HEAD；只有配置 bucket 内对象存在、`Content-Length` 与 `x-tos-meta-honcut-sha256` 均精确一致时，才可跳过重复 PUT 并签发新的短期 GET URL。显式业务对象键、长度/哈希不符、缺失 hash metadata、HEAD 非 200 或无法验证时仍走权威 PUT；该复用不是 retry，也不得把未确认对象解释为成功。

TOS 的连接、读取、写入、连接池和只读对账 timeout 只由 Runtime `TOSUploadExecutionPolicy` 配置，生产 Lifecycle 通过 `utils.provider_request_guard` 的独立媒体上传作用域传给 transport；client 不得读取 Phase 重试参数。图片压缩/格式规范化必须先完成，再对最终传输字节计算 object key、内容 hash 和账本身份。每次权威 PUT 前，`runtime.tos_uploads` 必须把唯一 `SubmissionAttempted` 原子写入 run-local `TOS_UPLOAD_LEDGER.json` 并置为 `submission_uncertain`。PUT 返回 200/201 后仍须用同一 object key 校验长度和 `x-tos-meta-honcut-sha256`；写超时、断连或响应丢失时只允许追加一次只读 HEAD 对账，严禁再次 PUT。HEAD 确认精确内容时写 `reconciled_completed`，否则保留 `submission_uncertain` 并 fail closed；恢复只能再次只读核对，不能自动重提。明确 HTTP 拒绝写 `provider_rejected`。账本只保存 object key、MIME、字节数、内容 hash、错误类型和转换事件，不得保存凭据、签名 URL、媒体正文或 Provider 响应正文；未知未来 schema、硬上限变化、已完成收据与远端对象不一致全部 fail closed。
...
Lifecycle 与各 Phase 的 ETA 由 Runtime `phase_estimates` owner 生成。Phase 1 尚未写出 canonical `CHARACTERS.json` 与 `STORYBOARD.json` 时，只能声明全程工作量待结构化，不得再以固定“三个角色、十个镜头”承诺总耗时。结构化工作量 `honcut.pipeline-workload-estimate.v1` 必须分别计数缺失的角色四视图、身份道具细节板、存在结构化战斗/持道具动作时每个相关角色至多一张动作板、Pxx 格、Sxx 总板与每个 Sxx 一张 cinematic first-frame；旧剧情状态变体不计入工作量。图片阶段以实际 `SEEDREAM_MIN_INTERVAL` 乘请求数形成基线。续跑估算只包含 CLI 选中且未由可信 checkpoint 完成的 Phase。Phase 5 基线与最多三轮配置上限内的全目标 Pxx/Sxx 补画必须显示为有界区间，不能把“无纠偏”历史均值冒充唯一 ETA。该估算是可观测性收据，不改变 Provider 请求、缓存或重试决策；缓存命中可令实际耗时低于基线，Provider/QA 失败也不得因 ETA 而被解释为成功。
...
Seedance 2.0 普通主镜头使用固定的全模态参考合同：canonical visual contract 先作为结构化 Prompt 权威注入；Phase 6 冻结最终媒体后，实际顺序为当前 Pxx 可见角色各一张四视图身份板 → P01 的 Phase 4 成片首帧或 P02+ 的真实前序视频 → 当前动作局部图 → 可选精确终态图 → 选定的一种身份中立 pose-atlas 包装 → 必要尾帧锚点。所有普通输入图片写为 `role=reference_image`，连续前序媒体写为 `role=reference_video`；必要资产超过九张图片时提交前 fail closed，不得静默裁减。Prompt 必须在最终 `content[]` 顺序确定后再生成唯一一组“图片 N / 视频 N”索引，不假设首帧固定为图片1。使用 pose atlas 时，该索引之后必须立即给出当前 Pxx 的动作权威：零时长首帧锚点不得占用生成时长，每个 canonical action group 依序且仅出现一次，正文明确可观察的根位移、重心、接触、道具互动、完成窗口和终态；后续 Pxx 动作不得进入简报。角色身份、连续性、风格和负向限制均为动作之后的支持约束，不能把开场姿态提升为持续姿态。单次请求只能有一套动作时钟和一条主运镜；live acceptance 不得在尾部追加第二套 pacing 合同。导演总览与导演 Sxx 单格只用于生成干净首帧，不直接进入 Seedance；Pxx PREVIS 和模型绘制的审核九宫格也不进入 Seedance。Phase 2 的审核九宫格仍保留 `Sxx_G01～G09`、红色动作箭头、蓝色运镜箭头和空间/视线/动作标识，但 Phase 6 只接收由 canonical action units、body-action mechanics 和 Adaptation 运镜合同本地编译、固定几何 renderer 绘制且 `source_pixel_usage=none` 的身份中立 atlas。最终索引绑定必须附加媒体职责：atlas 只拥有 Gxx 顺序、关节姿态、几何轨迹、箭头、时间包络和空间关系，不拥有人物脸、发型、妆造、服装、体型或道具外观；角色身份只服从四视图板，开场/连续状态只服从成片首帧或真实前序媒体，当前姿态、握持和道具几何只服从当前动作图及 canonical 文字。动作图中的多个人形必须解释为同一角色的参考姿态，禁止生成克隆、分栏、网格、文字、序号、箭头、边框或提前演绎后续动作。`prop_detail_board`、`identity_detail`、历史剧情状态图和 guide v1/v2/v3 均不得进入视频媒体。authority manifest、最终职责/索引、全部媒体 hash、Prompt hash、动作简报 hash、有序 group/source lineage、projection policy hash、Gxx/Axx、time/camera/atlas hash 和 canonical hash 必须进入任务 fingerprint。Provider-deny 复演只可读取并逐项验证持久化媒体、canonical、continuity 和 receipt hash，在源 run 外写独立收据；连续十次重建必须保持 Prompt、简报、媒体和任务身份一致且 Provider 请求为零。只有需要严格端点像素约束的 FLF2V 与跨镜 bridge 才使用 `first_frame/last_frame`，且跨 Sxx bridge 不得混入任何剧情图集、角色参考或动作姿态图。
...
Phase 2 的 Pxx、模型绘制 Sxx 审核九宫格与跨一级镜头 bridge 都必须在相同语义输入的普通重入和 Phase 3 character-lock 刷新中复用。每个 Sxx 审核九宫格固定 3×3，九个格子依阅读顺序绑定唯一 `Sxx_G01～G09`；Phase 2 按已声明 Pxx 顺序确定性、完整且无重叠地分配九格。Phase 2 pose compiler 将每个 Gxx 绑定当前 Pxx 的有序 generation/source action-unit、performer/target、阶段、时序角色与匹配的 body-action mechanics，再由 `honcut.identity-neutral-story-guide-renderer.v2` 按归一化关节几何绘制 `honcut.storyboard-narrative-guide.v4`：不同的动作或非静态阶段不得塌缩为同一 pose fingerprint；多人交互使用确定性角色槽位，非人物运动使用抽象物体符号，禁止凭空生成人形。红箭头只来自已解析动作向量，蓝箭头只来自 camera-motion contract；renderer 不复制审核板源像素，不绘制脸、发型、服装纹理、道具外观或剧情文字。shot-storyboard manifest v7 必须同时记录原审核网格和 `honcut.storyboard-pose-atlas-plan.v1` 的 action groups、pose samples、时间/运镜 hash、单图/分页候选、逐页图像 hash 与 `source_pixel_usage=none`。v6 及更早的可验证合同只允许零 Provider 并排迁移；旧资产不覆盖，血缘不完整时只能 audit-only，未知未来 schema 一律 fail closed。当前 v7 若只因 pose policy hash 落后，可由 Phase 2 migration owner 执行 `verified_v7_pose_policy_refresh_v1`：写入任何新生产绑定前必须验证旧 3×3 网格、审核板与逐项来源 hash、Gxx→Pxx 分配、guide/atlas 自身 hash 以及 canonical action-unit 血缘；验证通过后只能在按源 manifest SHA-256 隔离的目录中本地重编 pose contract、身份中立导航图和 atlas，Provider 请求数固定为零，并将旧 guide、receipt、atlas 与源 manifest 的逐文件 hash 副本登记为 audit-only。任一旧证据不完整、被篡改或动作血缘漂移时保持旧 manifest/资产不变，只写 audit-only 失败收据并 fail closed。Sxx cache 至少绑定当前 Prompt SHA-256、模型、尺寸、宽高比、图片请求合同版本、结构化 Phase 5 纠偏 attempt/issue、可解码文件、内容 SHA-256 与完整 grid receipt；任一项变化或损坏时只重生成该 Sxx，不得连带重提已验证的 Pxx。

动作图集中的 Gxx 是 pose sample，不等同于独立剧情动作。Phase 2 先以当前 Pxx 的 canonical generation/source action lineage 形成有序 action groups，再按版本化 Provider capability profile 分配单调姿态样本；4/7/10/15 秒的 Seedance 2.x 初始 profile 分别提供 9/18/27/36 个样本与 6/10/15/20 个可靠动作组上限，超过动作组上限必须返回 Adaptation 拆分 Pxx，禁止靠增加格子压缩或改写剧情。`honcut.storyboard-action-timing.v1` 将 P01 的首帧锚点记为零故事时间，为动态动作保存完成窗口，并默认保留 0.8～1.5 秒的 `semantic_hold` 终态稳定；只有显式 `exact_pose` 才增加一张带 hash 的终态参考并占用媒体预算。Phase 2 从同一 pose payload 零调用预渲染 `single_atlas` 和适用的九格分页候选；Phase 6 冻结身份板、成片首帧/前序视频、动作局部图、精确终态图和尾帧锚点后，优先选择能完整容纳的分页候选，否则选择单张密集 atlas，仍超过九图则提交前 fail closed。分页之间必须完整、有序、无重叠地覆盖 Gxx，跨 Sxx bridge 不得携带任何 atlas。

运镜技术选择仍唯一归 Phase 1 Adaptation。`camera_motion_contract` v3 可持久化机位高度、轨道位移/速度、焦段起止/变化速度、pan/tilt 角度与角速度、分段数和停顿，并保存合同 hash；Phase 2 只能沿该单一连续路径投影各 pose sample 的视角、关节压缩和遮挡次序，禁止另选或修复运镜。基于位移/速度、角度/角速度、焦段变化和段间停顿计算的最小时长若超过 Pxx 时长，必须在任何 Provider 工作前返回 Adaptation。
...
来源索引 Adaptation 修复使用独立的 `pipeline/scripts/adaptation_rewrite_live_acceptance.py`。输入只能是哈希验证后的外部 `honcut.adaptation-rewrite-live-input.v1`，包含来源事件、Director 计划和正式时长/布局参数；零提交预检必须通过同一 Adaptation owner 确定 production event、固定 rewrite group、事件顺序和请求投影，并验证当前提交的 `honcut.adaptation-rewrite-regression.v1`。获当次费用授权后，入口只执行 `_apply_director_action_selection()` 中一个真实 Ark source-indexed rewrite 请求：Runtime 统一设 `max_retries=0`，Ark POST 前原子写 `submission_uncertain`，失败、超时、schema 拒绝或进程中断均禁止重提。业务 verdict 校验 reconciliation 后的 ID 与预检固定顺序一致、`source_fact_loss_count=0`，持久化结果只含事件索引和内容哈希，不保存模型 prose、Prompt 或完整响应。该窄入口不得被 Graph、Lifecycle 或普通生产 CLI 引用，也不得读写失败 run 的 checkpoint/cache/Artifact。

Director sequence 归并修复使用独立的 `pipeline/scripts/director_plan_live_acceptance.py`。输入只能是哈希验证后的外部 `honcut.director-plan-live-input.v1` 来源事件合同；零提交预检必须由同一 Director owner 固定来源 sequence ID、顺序、事件哈希和请求投影，并验证当前提交的 `honcut.director-plan-reconciliation-regression.v1`。获当次费用授权后，入口在隔离目录中最多执行一次真实 Ark Director 请求：Runtime 统一设 `max_retries=0`，Ark POST 前原子写 `submission_uncertain`，失败、超时、schema 拒绝或进程中断均禁止重提。业务 verdict 要求 canonical ID 与来源顺序完全一致、五项意图均非空、`source_sequence_loss_count=0`；持久化验收证据只含 ID、位置和内容哈希，不保存模型 prose、Prompt 或完整响应。该窄入口不得被 Graph、Lifecycle 或普通生产 CLI 引用，也不得读取或修补失败 run 的 checkpoint/cache/Artifact。
...
36 秒全链路 Stage 0 还必须单独冻结 TOS 权威 PUT 上限；Phase 1 后按 canonical 角色与镜头冻结剩余图片/VLM/TOS 工作量，Phase 5 后按真实 Pxx/bridge 冻结视频及其输入上传工作量。TOS 计数只取 `TOS_UPLOAD_LEDGER.json` 的唯一 `SubmissionAttempted`，顶层失败汇总不得把已发生的对象上传写成零请求；任一 `submission_uncertain`、`provider_rejected`、未知账本版本或硬上限漂移都会停止全链路。

TOS 上传修复使用独立的 `pipeline/scripts/tos_upload_live_acceptance.py`。输入必须是隔离验收目录内、哈希验证后的 1–10 MB 合法多模态图片，并由绑定当前提交的 `honcut.tos-upload-regression.v1` 证明零请求回归已通过。默认入口只写 `honcut.tos-upload-live-acceptance.v1` 预检，固定唯一 object prefix、一次权威 PUT、最多两次只读 HEAD 和零其他 Provider 请求；获得当次授权后才使用 `--submit`。提交前验收收据与 `TOS_UPLOAD_LEDGER.json` 都必须落为 `submission_uncertain`；成功 PUT 或写超时后精确 HEAD 对账均可形成调用链通过，未验证、拒绝、中断或重复运行必须终止且禁止重提。该入口不得调用 Seedream、Ark VLM 或 Seedance，也不得进入生产 Graph/Lifecycle。

Phase 1 后的外部人物期望只能通过已验证 Character Roster 的 entity `display_name` 与 instance `source_mentions` 解析，并且必须唯一命中。`source_visual_evidence.source_excerpt` 是可包含多名同场人物的视觉事实证据，不具有人物别名 authority，不得参与人物身份锚点匹配；真正的名册标签歧义继续 fail closed。

36 秒验收的 Phase 1 必须在独立进程内安装 `provider_attempt_scope(max_retries=0)`；该 Runtime 作用域是跨工作线程的，同时收紧 Event Extractor、角色发现、Director 计划、Adaptation、Storyboard Prompt、Ark 限流/网络重试、Seedream 配额重试与导演故事板版式重生。Ark 和 Seedream 传输边界必须在每次网络请求前向 Phase 1 有限账本写入只含模型、规格和 Prompt/媒体 hash 的安全 payload；并发请求可在当前进程中同时处于 `submission_uncertain`，但进程恢复后的任一未决请求或已知失败都必须禁止新提交。作用域外的生产默认重试上限不改变，也不得以普通 CLI 或环境变量开启该验收策略。
...
Phase 3 动作板修复复用专用入口 `pipeline/scripts/phase6_storyboard_guide_live_acceptance.py`，但收据合同升级为 `honcut.phase3-performance-board-live-acceptance.v1`：它只从至少含两个 Pxx、P01 可见角色为一至两人、且 P01 存在绑定道具的当前动作图之 Sxx 投影一个请求。零提交预检必须验证 v3 合成人身份及当前审美档案 ID/哈希、整板/局部图/收据哈希、最终媒体顺序、Gxx/Axx/source action-unit/道具、Prompt、预算和 generation fingerprint；隔离 `GenerationTaskStore`、逻辑 submit guard 与原始 Seedance POST guard 共同限制为最多一次真实提交。regression evidence 可在未提交时独立落盘为 pass，整体仍保持 `pending_live_acceptance`；调用链成功后仍保持业务 verdict 待人工审查。只有同一 Git 提交的 regression evidence 与“从指定首帧开始、珍珠生体瓷妆明确、温暖健康且美观，眼神清醒有眼神光、无灰白尸感或恐怖谷神态、人物一致不克隆、道具正确、只演绎当前 Gxx/Axx、成片无文字/箭头/边框/网格”的业务 verdict 均为 pass，Phase 1～Phase 9 收据才可写为 `accepted`。
