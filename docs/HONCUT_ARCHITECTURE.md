...
Phase 2 pose contract v4、Phase 3 performance pose contract v3、Phase 6 action-execution brief v2 与隔离的 motion-blueprint v6 都只消费上述同一投影及 hash，不得重新解析自由动作 prose 或维护平行动作模板。Phase 2 必须把通道数值实际投影到人物骨架像素；Phase 3 只输出当前 performer/GAU 的局部姿态约束；Phase 6 仅投影紧凑的有序 phase、active channels、朝向、transform 与 hash，并保持既有媒体职责和九图预算。motion blueprint 仍是零费用隔离能力门，不得作为普通 Phase 6 `reference_video` 或其他媒体进入生产请求；旧 technique registry 只能作为 immutable audit compatibility。已知 v1 body-action 只有在父 Event/Storyboard Artifact、完整 mechanics、source hash 与最终 GAU/Pxx 血缘全部可验证时，才允许零 Provider 生成并排 `honcut.action-kinematics-sidecar.v1`；原父资产保持不变并转 audit-only，其下游 pose/atlas 资产标记 stale/audit-only。证据不完整、损坏或未来 schema 必须要求 owner 重建，禁止从 prose 猜测或局部刷新。
