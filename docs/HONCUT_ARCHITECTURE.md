...
Adaptation Engine 是具体镜头语言的唯一 owner。除 `shot_size`、`camera_movement`、`lens_mm`、`lighting_key` 等既有字段外，canonical shot 必须写入受控 `camera_angle`（`eye_level/low/high/dutch/over_shoulder/aerial/bird/worm`）；Director 意图不得预选该值。下游 Storyboard、二级 Pxx（当前 `honcut.secondary-storyboard.v13`）、Phase 4 元数据与生成 Prompt 只能保留或翻译这个 canonical 值，不得再次推断角度。每个 canonical shot 必须按 `source_events` 顺序持久化 `source_event_casts`，Storyboard Generator 必须原样复制该结构化血缘而不得通过字段白名单丢弃；每个 Pxx 必须写与本格 `generation_action_units` 一致的 `source_event_ids`，并由这些事件的 canonical cast 与本格显式身份事实共同投影 `character_ids`。这使省略主语的动作仍保留结构化执行者，同时无角色事件不会从 Sxx 级角色全集继承尚未入场的人物；Sxx 级角色全集始终只是上限。来源事件、动作单元、角色 ID 缺失/重复/越界或血缘不一致必须在 Phase 1 fail closed。Phase 2 只接受当前 v13 canonical cast；v11 仅可走既有显式可见事实迁移，已知可能漏掉隐式主语的 v12 与未知版本即使带有 `character_ids` 也必须 fail closed。生产代码只写 v13 canonical 字段。
