...
Phase 2 的 Pxx、模型绘制 Sxx 审核九宫格与跨一级镜头 bridge 都必须在相同语义输入的普通重入和 Phase 3 character-lock 刷新中复用。每个 Sxx 九宫格固定 3×3，九个格子依阅读顺序绑定唯一 `Sxx_G01～G09`；Phase 2 按已声明 Pxx 顺序确定性、完整且无重叠地分配九格。Phase 2 pose compiler 将每个 Gxx 绑定当前 Pxx 的有序 generation/source action-unit、performer/target、阶段与匹配的 body-action mechanics，再由 `honcut.identity-neutral-story-guide-renderer.v2` 按归一化关节几何绘制 `honcut.storyboard-narrative-guide.v3`：不同的动作或非静态阶段不得塌缩为同一 pose fingerprint；多人交互使用确定性角色槽位，非人物运动使用抽象物体符号，禁止凭空生成人形。红箭头只来自已解析动作向量，蓝箭头只来自 camera-motion contract；renderer 不复制审核板源像素，不绘制脸、发型、服装纹理、道具外观或剧情文字。shot-storyboard manifest 必须记录 pose schema/policy hash、逐格 pose contract/fingerprint、语义 payload hash、Gxx→Pxx 分配、源审核板 hash、派生图 hash 与 `source_pixel_usage=none`。guide v1/v2 只有在完整九格合同、源 hash、Pxx 分配、收据与 canonical action lineage 全部可验证时，才允许零调用并排归档旧证据后重绘为 v3；否则旧资产只能 audit-only 并要求从 Phase 2 重建，未知未来 schema 一律 fail closed。Sxx cache 至少绑定当前 Prompt SHA-256、模型、尺寸、宽高比、图片请求合同版本、结构化 Phase 5 纠偏 attempt/issue、可解码文件、内容 SHA-256 与完整 grid receipt；任一项变化或损坏时只重生成该 Sxx，不得连带重提已验证的 Pxx。
