...
def write_checkpoint(path: str|Path, stage: str, result: dict[str, Any], *, run_fingerprint: str|None = None) -> dict[str, Any]:
    ...
    if run_fingerprint is not None and value.get("run_fingerprint") not in (None, run_fingerprint):
        value={"completed": [], "results": {}, "timestamp": "", "run_fingerprint": run_fingerprint}
    ...
    if run_fingerprint is not None: value["run_fingerprint"] = run_fingerprint
