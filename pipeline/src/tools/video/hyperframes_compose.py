...
class HyperFramesCompose(BaseTool):
    ...
    def _scaffold(self, inputs: dict[str, Any]) -> ToolResult:
        """Materialize the HyperFrames workspace from HonCut artifacts.
        ...
        html = self._generate_index_html(
            ...
            or f"HonCut {edit_decisions.get('renderer_family', 'composition')}",
        )
    ...
    def _resolve_dimensions(
        ...
        if profile_name:
            try:
                from utils.media_profiles import get_profile  # type: ignore
    ...
    def _style_bridge(
        ...
        """Bridge HonCut playbook → HyperFrames CSS vars + DESIGN.md.
        ...
        """
        try:
            from utils.hyperframes_style_bridge import style_bridge  # type: ignore
        ...
        design_md = (
            "# DESIGN\n\n"
            "Generated by HonCut HyperFrames style bridge (fallback).\n\n"
            f"- Background: `{bg}`\n"
            f"- Foreground: `{fg}`\n"
            f"- Accent: `{accent}`\n"
            f"- Primary: `{primary}`\n"
