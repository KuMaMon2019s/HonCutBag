...
|-------|------|-------------|
| Phase 1 | Screenwriter Engine | Text parsing → event extraction → sequence-aware Director intent (`scene_goal`, `emotion_arc`, `visual_focus`, `spatial_intent`, `transition_intent`) → character discovery → layered cinematic adaptation → continuity-boundary classification → per-shot storyboard JSON with eight-layer prompts. Director explains why a sequence should be filmed; Adaptation remains the sole owner of shot count, framing, movement, lighting, and duration. Sub-phase checkpoints and a 15s heartbeat keep it observable |
