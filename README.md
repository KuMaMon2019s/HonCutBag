...
| Phase 2 | Storyboard Images | Per-Pxx PREVIS plus one annotated 3×3 review grid per Sxx; each Pxx also gets deterministic 9/18/27/36-sample identity-neutral pose-atlas candidates derived from its duration and canonical action lineage |
...
| Phase 4 | Scene Consistency | Shot layout, scene/continuity contracts, and exactly one clean cinematic P01 frame per Sxx; P02+ extends the predecessor video without generating another first frame |
...
| Phase 6 | Video Generation | After freezing authoritative identity, boundary, action and continuity media, P01/P02+ select exactly one pre-rendered single or paged pose-atlas strategy within the nine-image budget and submit it through the crash-safe runtime |
...
- **Adaptive storyboard pose atlases** — Provider capability profiles separate semantic action-group capacity from pose-sample density. A 4/7/10/15-second Pxx deterministically plans 9/18/27/36 pose samples, keeps the opening reference pose at zero story time, gives dynamic action an explicit completion window, and treats the final 0.8–1.5 seconds as a semantic terminal hold. Phase 2 locally renders both dense and paged candidates from one canonical payload; Phase 6 chooses one after the final media budget is known, without a Provider call or plot-specific constants.
